...
type Metrics struct {
	...
	segmenterSegments      *prometheus.CounterVec
	segmenterRevisions     prometheus.Counter
	segmenterBoundaryDelay *prometheus.HistogramVec
...
func NewMetrics() *Metrics {
	m := &Metrics{
		...
		segmenterSegments:      prometheus.NewCounterVec(prometheus.CounterOpts{Name: "tmk_segmenter_segments_total", Help: "Segments emitted by strategy and reason."}, []string{"strategy", "reason"}),
		segmenterRevisions:     prometheus.NewCounter(prometheus.CounterOpts{Name: "tmk_segmenter_partial_revisions_total", Help: "Partial segment revisions emitted."}),
		segmenterBoundaryDelay: prometheus.NewHistogramVec(prometheus.HistogramOpts{Name: "tmk_segmenter_boundary_delay_seconds", Help: "Observed segment duration at finalization."}, []string{"strategy"}),
	...
	m.registry.MustRegister(m.httpRequests, m.httpDuration, m.httpInFlight, m.websocketConnections,
		...
		m.storageBytes, m.storageFreeBytes, m.applicationReady, m.segmenterSegments, m.segmenterRevisions, m.segmenterBoundaryDelay, m.build, prometheus.NewGoCollector(), prometheus.NewProcessCollector(prometheus.ProcessCollectorOpts{}))
	m.build.WithLabelValues(buildinfo.Version, buildinfo.Commit).Set(1)
	return m
}

func (m *Metrics) Segment(strategy, reason string, partial bool, duration time.Duration) {
	if strategy == "" {
		strategy = "unknown"
	}
	if reason == "" {
	...
	m.segmenterSegments.WithLabelValues(strategy, reason).Inc()
	if partial {
		m.segmenterRevisions.Inc()
	}
	if duration > 0 {
		m.segmenterBoundaryDelay.WithLabelValues(strategy).Observe(duration.Seconds())
	}
}
